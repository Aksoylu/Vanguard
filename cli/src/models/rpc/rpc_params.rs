use crate::{
    ...
    models::rpc::rpc_payload::RPCPayload,
    utils::crypt_utility::{encrypt_aes256_hex, generate_nonce_hex},
};
...
impl RPCParams {
    pub async fn build(payload: RPCPayload) -> Result<Self, RPCBaseError> {
        let lock = {
            let rpc_client = RPC_CLIENT.read().await;

            let serialized_json = serde_json::to_string(&payload)
                .map_err(|_| RPCBaseError::build("Can not serialized RPC payload"))?;
            ...
            if aes_encryption_key.is_none() {
                return Err(RPCBaseError::build(
                    "Encryption key can not found in session data",
                ));
            }
            ...
            if encrypt_payload.is_none() {
                return Err(RPCBaseError::build(
                    "Encryption failed, please check session information",
                ));
            }
