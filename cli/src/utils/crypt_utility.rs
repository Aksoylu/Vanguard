...
pub fn generate_nonce_hex() -> String {
    // 1. Initialize an array of 12 zero bytes. This is the standard length for AES-GCM Nonce.
    let mut nonce = [0u8; 12]; 

    // 2. Fill the Nonce array with cryptographically secure random data
    //    using the thread-local random number generator.
    thread_rng().fill_bytes(&mut nonce);

    // 3. Convert the generated byte array to a Hex string and return it.
    hex::encode(nonce)
}
...
pub fn encrypt_aes256_hex(key_hex: &str, plaintext: &str, nonce_hex: &str) -> Option<String> {
    ...
    return Some(STANDARD.encode(ciphertext))
}
